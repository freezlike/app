                            ...
                            <?php foreach ($factures as $user): //debug($user); die(); ?>
                                <?php if ($user['User']['commerciale_id'] > 0 && $user['User']['full_name'] !== null): ?>
                                    <?php if ($user['User']['user_count'] > 0 && $user['User']['full_name'] !== null): ?>
                                        <tr class="gradeX">
                                            <!-- <td><?php //echo $user['User']['full_name'] . " - " . $user['Societe']['raison_social'] ;    ?></td> -->
                                            <td><?php echo $user['User']['full_name']; ?></td>
                                            <td><?php echo $user['Facture']['code_facture']; ?></td>
                                            <td><?php echo $user['Commercial']['full_name']; ?></td>
                                            <td><?php echo $user['Facture']['created']; ?></td>
                                            <td><?php echo $user['Facture']['limit_date']; ?></td>
                                            <td>
                                                <a class="btn btn-primary btn-sm" href="<?php echo $this->Html->url(array('controller' => 'factures', 'action' => 'view', 'admin' => false, $user['Facture']['id'])); ?>"><i class="glyphicon glyphicon-play"></i></a>
                                                <?php if ($user['Facture']['avoir'] == 1): ?>
                                                    <label class="label label-info"><?php echo __('Facture en Avoir'); ?></label>
                                                <?php else: ?>
                                                    <a class="label label-danger" href="<?php echo $this->Html->url(array('controller' => 'factures', 'action' => 'admin_avoir', 'admin' => false, $user['Facture']['id'])); ?>">
